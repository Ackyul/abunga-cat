...
const Profile = () => {
  ...
  const { 
    user, loading, updateProfile, disconnectGoogle, disconnectFacebook, logout, checkSession, claimWelcomeGift 
  } = useAuthStore();
  ...
  // Manejar query strings de redirecciones de OAuth Connection (Google & Facebook)
  useEffect(() => {
    const query = new URLSearchParams(location.search);
    if (query.get("connect_success") === "true") {
      toast.success("¡Tu cuenta de Google se ha conectado correctamente!");
      navigate("/profile", { replace: true });
    } else if (query.get("connect_facebook_success") === "true") {
      toast.success("¡Tu cuenta de Facebook se ha conectado correctamente!");
      navigate("/profile", { replace: true });
    } else if (query.get("connect_error") === "email_already_linked" || query.get("connect_error") === "already_linked") {
      toast.error("Este correo ya está vinculado a otra cuenta.");
      navigate("/profile", { replace: true });
    } else if (query.get("connect_error") === "facebook_already_linked") {
      toast.error("Este correo de Facebook ya está vinculado a otra cuenta.");
      navigate("/profile", { replace: true });
    } else if (query.get("connect_error") === "true") {
      toast.error("Error al conectar tu cuenta social.");
      navigate("/profile", { replace: true });
    }
  }, [location, navigate]);
  ...
  const handleConnectFacebook = () => {
    window.location.href = "/api/users/facebook?connect=true";
  };

  const handleDisconnectFacebook = async () => {
    if (!window.confirm("¿Seguro que deseas desvincular tu cuenta de Facebook?")) return;
    try {
      await disconnectFacebook();
      toast.success("Cuenta de Facebook desvinculada con éxito.");
    } catch (err) {
      toast.error("Error al desvincular cuenta de Facebook.");
    }
  };
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {activeTab === "settings" && (
              <div className="space-y-6">
                ...
                {/* Panel de Conexiones OAuth (Google & Facebook) */}
                <div className="bg-white rounded-3xl p-6 shadow-md border border-[#95b721]/15 text-left">
                  ...
                  <p className="text-xs text-gray-500 mb-6">
                    Vincula tus cuentas sociales para iniciar sesión rápidamente sin escribir tu contraseña.
                  </p>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-4 rounded-2xl bg-gray-50 border border-gray-150">
                      <div className="flex items-center gap-3">
                        <svg className="w-6 h-6 flex-shrink-0 fill-[#1877F2]" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      ...
                      {user?.facebook_email ? (
                        ...
                        <button
                          ...
                          className="flex items-center text-xs font-bold text-[#1877F2] hover:text-[#166fe5] bg-[#1877F2]/10 hover:bg-[#1877F2]/20 border border-[#1877F2]/30 px-3 py-1.5 rounded-xl transition cursor-pointer"
                        >
                          <Link2 size={14} className="mr-1.5" />
                          Vincular cuenta
                        </button>
